                    ...
                    <form method="POST" action="{{ route('store-property') }}">
                        ...
                        <div class="form-group">
                            ...
                            <select id="billing-day" class="form-control" name="billing_day" required>
                                ...
                                @for ($i = 1; $i <= 31; $i++)
                                    <option value="{{ $i }}" {{ old('billing_day') }}>
                                        {{ $i }}{{ $i == 1 ? 'st' : ($i == 2 ? 'nd' : ($i == 3 ? 'rd' : 'th')) }}
                                    </option>
                                @endfor
                            </select>
                            <small class="form-text text-muted">Select a day for the billing period (e.g., 20th). The system
                                will calculate the billing period from this day to the same day next month.</small>
                        </div>
                        <div class="form-group">
